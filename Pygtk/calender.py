...
class CalendarExample:
  ...
  def calendar_month_changed(self,widget):
    buffer="month changed: %s" %self.calendar_date_to_string()
    self.calendar.set_signal_strings(buffer)

  def calendar_day_selected(self,widget):
    buffer="day selected:%s" % self.calendar_date_to_string()
    self.calendar_set_signal_strings(buffer)

  def calendar_day_selected_double_click(self,widget):
    buffer="day selected double click:%s"% self.calendar_date_to_string()
    self.calendar_set_signal_strings(buffer)

    year,month,day=self.window.get_date()

    if self.marked_date[day-1]==0:
      sekf.window.mark_day(day)
      self.marked_date[day-1]=1
    else:
      self.window.unmark_day(day)
      self.marked_date[day-1]=0
  def calendar_prev_month(self,widget):
    buffer="prev month:%s" % self.calendar_date_to_string()
    self.calendar_set_signal_strings(buffer)
  def calendar_next_month(self,widget):
    buffer="Next Month:%s"% self.calendar_date_to_string()
    self.calendar_set_signal_strings(buffer)
  def calendar_prev_year(self,widget):
    buffer="Prev Year:%s"%self.calendar_date_to_string()
    self.calendar_set_signal_strings(buffer)
  def calendar_next_year(self,widget):
    buffer="next year:%s"%self.calendar_date_to_string()
    self.calendar_set_signal_strings(buffer)
  ...
  def __init__(self):
    flags=["Show Heading","Show Day Names","No Month Changes","Show Week Numbers"]
    self.window=None
    self.font=None
    self.font_dialog=None
    ...
    vbox3=gtk.VBox(True,self.DEF_PAD)
    frame.add(vbox3)
